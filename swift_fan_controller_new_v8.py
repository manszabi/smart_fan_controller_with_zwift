...
class HUDWindow(QWidget):
    ...
    def __init__(self, controller: "FanController", app: "QApplication") -> None:  # type: ignore[reportInvalidTypeForm]
        ...
        self._lbl_zone.setMinimumHeight(30)
        ...
        zone_frame.setMinimumHeight(50)
        content_layout.addWidget(zone_frame, 1)  # stretch=1: prioritás a zone-nak
    ...
    def _update(self) -> None:
        try:
            ...
            if zwift is not None and (power_zwift or hr_zwift):
                ...
                self._update_label(
                    self._lbl_zwift_udp,
                    "RECEIVING" if ok else "NO SIGNAL",
                    self.LCARS_CYAN if ok else self.LCARS_RED,
                )
            else:
                self._update_label(
                    self._lbl_zwift_udp, "\u2013 \u2013 \u2013", self.TEXT_DIM
                )
            ...
            if ble_fan is not None and getattr(ble_fan, "last_sent_time", 0) > 0:
                ...
                self._update_label(
                    self._lbl_last_sent, "\u2013 \u2013 \u2013", self.TEXT_DIM
                )
    ...
    def _apply_scale(self) -> None:
        ...
        self._lbl_zone.setMinimumHeight(max(20, int(40 * s)))
        self._lbl_zone.setStyleSheet(
...
def main() -> None:
    ...
    def signal_handler(signum: int, frame: Any) -> None:
        ...
        cleanup()
        # PySide6 mainloop leállítása – ez a kulcs, különben a fő szál nem áll le
        hud = hud_ref[0]
        if hud is not None:
            try:
                # QApplication.quit() thread-safe a Qt-ben
